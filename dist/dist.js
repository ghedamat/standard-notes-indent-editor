...
function IndentEditor(target_textarea, indent_editor_options) {
  ...
  this.insertLink = function (cm) {
    ...
    for (var i = sels.length - 1; i >= 0; i--) {
      ...
      var line = anchor.line;
      var date = new Date();
      var link = date.toLocaleString("fr-CA", {
        year: "numeric",
        month: "2-digit",
        day: "2-digit",
        hour: "2-digit",
      ...
      cm.replaceRange("[[".concat(link, "]]") + cm.doc.lineSeparator(), {
        line: line,
        ch: 0
      }, {
        line: line,
  ...
  this.insertTime = function (cm) {
    ...
    for (var i = sels.length - 1; i >= 0; i--) {
      ...
      var line = anchor.line;
      var date = new Date();
      cm.replaceRange("# ".concat(date.toLocaleDateString("fr-CA")) + cm.doc.lineSeparator(), {
        line: line,
        ch: 0
      }, {
        line: line,
  ...
  CodeMirror.defineMode("indent_text", function (cmCfg, modeCfg) {
    ...
    var mode = {
      ...
      token: function token(stream, state) {
        ...
        if (!state.sawTextBeforeOnLine) {
          var incomplete = matchRegexToString(stream, /=/, true);
